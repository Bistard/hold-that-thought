...
function parseTime(input: string): number {
	...
	if (fullMatch) {
		return new Date(
			`${fullMatch[1]}T${fullMatch[2].padStart(2, '0')}:${fullMatch[3]}:00`,
		).getTime();
	}
...
function handleCommand(input: string, manager: BufferManager, rl: readline.Interface): void {
	...
	switch (cmd) {
		case 'export': {
			...
			if (args.output) {
				try {
					...
					console.log(
						chalk.red(`错误: 无法写入文件 ${args.output} — ${(err as Error).message}`),
					);
				}
			} else {
				console.log(output);
		...
		case 'status': {
			...
			const dbStr = range ? `${formatTime(range.min)} ~ ${formatTime(range.max)}` : '空';
